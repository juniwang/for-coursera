...
class Ship:
    ...
    def update(self):
        ...
        self.vel[0] = 0.95 * self.vel[0] + self.acceleration[0]
        self.vel[1] = 0.95 * self.vel[1] + self.acceleration[1]
        self.pos[0] = (self.pos[0] + self.vel[0]) % WIDTH
        self.pos[1] = (self.pos[1] + self.vel[1]) % HEIGHT
        
    def shoot(self):
        # shoot a missile. The missile's initial position should be the tip of your ship's "cannon". 
        # Its velocity should be the sum of the ship's velocity and a multiple of the ship's forward vector.
        global a_missile
        forward_vector = angle_to_vector(self.angle)
        missile_factor = 5
        missile_pos = [self.pos[0] + forward_vector[0] * self.radius, 
                       self.pos[1] + forward_vector[1] * self.radius]
        missile_vel = [self.vel[0] + forward_vector[0] * missile_factor,
                       self.vel[1] + forward_vector[1] * missile_factor]
...
def draw(canvas):
    ...
    # draw scores
    canvas.draw_text("lives: "+str(lives), [WIDTH*4/5, HEIGHT/12], 30, "yellow")
    canvas.draw_text("score: "+str(score), [WIDTH*4/5, HEIGHT/6], 30, "yellow")
...
def rock_spawner():
    ...
    angel_vel = random.random() * 0.4 - 0.2
    
    a_rock = Sprite(pos, vel, 0, angel_vel, asteroid_image, asteroid_info)
